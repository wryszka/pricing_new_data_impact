...
# MAGIC %md
# MAGIC # Model Review Agent — Interactive Model Governance Assistant
# MAGIC
# MAGIC This notebook creates an **interactive AI agent** that actuaries can use to query and
...
# MAGIC %md
# MAGIC ## 1. Load Model Factory Results
...
def compare_models(model_a_name: str, model_b_name: str) -> pd.DataFrame:
    ...
    # Compute difference / improvement where sensible
...
def drop_feature_impact(feature_name: str) -> pd.DataFrame:
    ...
    # Check feature impact table first
    fi_row = feature_impact_df[feature_impact_df["feature"].str.lower() == feature_lower]

    # Find models whose description or name references this feature
    mask_with = (
...
def model_detail(model_name: str) -> dict:
    ...
    # Attach coefficients if available
...
SYSTEM_PROMPT = """You are an expert actuarial assistant helping insurance pricing actuaries review
...
| Deviance explained | Higher is better | % of null deviance explained (like R²) |
...
def build_full_system_prompt() -> str:
    ...
    return SYSTEM_PROMPT.format(data_summary=data_summary)
...
def query_agent(question: str, max_tokens: int = 1024, verbose: bool = True) -> str:
    ...
    # Primary model: Claude Sonnet 4 — fallback to Llama 3.1 70B if unavailable
...
# MAGIC Each cell shows both the query and the agent's response.

# COMMAND ----------

# MAGIC %md
# MAGIC ### 5.1 — Top Models by AIC

# COMMAND ----------

# First, show the raw helper output for reference
print("Helper output — top_models(5, 'aic'):")
display(top_models(5, "aic"))

# COMMAND ----------

_ = query_agent("What are the top 5 models by AIC? Summarise what features they use and whether the improvement over the baseline is meaningful.")

# COMMAND ----------

# MAGIC %md
# MAGIC ### 5.2 — Feature Unavailability Scenario

# COMMAND ----------

# Show helper output first
...
# MAGIC %md
# MAGIC ### 5.3 — Baseline vs Best Model Comparison

# COMMAND ----------

# Identify baseline and best model names from the data
...
# MAGIC %md
# MAGIC ### 5.4 — Most Valuable Enrichment Features
...
# MAGIC %md
# MAGIC ### 5.5 — Model Governance Summary
...
# MAGIC %md
# MAGIC ## 6. Interactive Use
# MAGIC
# MAGIC The cells above demonstrate the agent with preset questions. To use the agent
# MAGIC interactively, simply call `query_agent()` with your own question:
# MAGIC
# MAGIC ```python
...
# MAGIC query_agent("Are there any features in the top models that might raise regulatory concerns for UK motor pricing?")
# MAGIC
